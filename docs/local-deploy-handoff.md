...
| 角色 | 路径 | Git 形态 | 说明 |
| --- | --- | --- | --- |
| 部署目录 | `/home/guxy/srv/deerflow` | detached HEAD | 只用于运行；不在此开发、不提交 |
...
两者共享同一 `.git`（`git worktree`），因此部署目录的基线切换用 `git -C /home/guxy/srv/deerflow checkout <sha>` 即可，无需重新 clone。
...
统一入口：`scripts/deerflow-local-ops.sh`（本仓库脚本，包装仓库既有 `scripts/deploy.sh`，
自动把每次操作的原始输出归档到 `<部署目录>/.local-ops/logs/`）。

```bash
DEPLOY=/home/guxy/srv/deerflow
OPS=/mnt/data/Codes/offcial/worktrees/RUYI-117/scripts/deerflow-local-ops.sh

bash "$OPS" up        # 构建镜像并启动（首次部署 / 基线变更后）
bash "$OPS" start     # 不重建直接启动（日常启动、重启恢复）
bash "$OPS" down      # 停止并移除容器（保留数据）
bash "$OPS" restart
bash "$OPS" status    # 容器 / 端口 / 资源 / 卷
bash "$OPS" health    # 健康探针
bash "$OPS" logs      # 跟随网关日志
bash "$OPS" uninstall # 完整卸载（二次确认）
```

等价的仓库原生入口（在部署目录内执行）：`make up` / `make down` / `bash scripts/deploy.sh start`。
...
若此前执行过 `down`（容器被移除），宿主重启不会恢复，需手动 `bash "$OPS" start`。
...
| `<部署目录>/.local-ops/secrets/` | 否（在部署目录内，非仓库路径） | 管理员账号与 cookie，`chmod 600` |
...
| gateway unhealthy | `bash "$OPS" logs` 看网关日志；确认 `.env` 中模型凭据存在 |
| 搜索工具超时 | 切换 `config.yaml` 中 `web_search` 的 `use:` 到可用后端 |
| 端口 2026 被占 | `.env` 中改 `PORT`，`bash "$OPS" restart` |
| 数据疑似损坏 | `down` 后备份 `backend/.deer-flow/data`，再排查 |

## 9. 完整卸载 / 回退

```bash
bash "$OPS" uninstall              # 容器 + redis 卷 + 本部署镜像
rm -rf /home/guxy/srv/deerflow/backend/.deer-flow   # 可选：清业务数据
